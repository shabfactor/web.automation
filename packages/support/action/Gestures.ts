...
class Gestures {
   ...
   * @param {number} amount
   */
  static checkIfDisplayedWithScrollDown(
    ...
    if (
      ...
      throw new Error(
        `The element '${element.ELEMENT}' could not be found or is not visible.`
      );
    }
  }
   ...
   * </pre>
   */
  static swipeOnPercentage(from: Coordinates, to: Coordinates): void {
   ...
   * </pre>
   */
  static swipe(from: Coordinates, to: Coordinates): void {
    driver.touchPerform([
      {
        action: 'press',
        options: from,
   ...
   * @private
   */
  static _getDeviceScreenCoordinates(
